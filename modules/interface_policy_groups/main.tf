...
resource "aci_leaf_access_port_policy_group" "access" {
  ...
  relation_infra_rs_l2_inst_pol            = each.value["relation_mtu_policy"]    # class: l2InstPol          DN: 'uni/infra/l2pol-{Name}'
  ...
  relation_infra_rs_macsec_if_pol          = each.value["relation_link_level"]    # class: macsecIfPol        DN: 'uni/infra/macsecifp-{Name}'
